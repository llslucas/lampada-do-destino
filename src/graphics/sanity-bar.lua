...
function SanityBar:draw()
  ...
  local avatar
  avatar = self.player.sanity >= 50 and self.imgNormal or self.imgMedo
  ...
  LG.rectangle("fill", self.x + 64 * IMAGE_SCALING, self.y + 35 * IMAGE_SCALING, 157/100 * self.player.sanity, 26 * IMAGE_SCALING)
  LG.setColor(1,1,1)
end

function SanityBar:setPlayer(player)
  self.player = player
end
